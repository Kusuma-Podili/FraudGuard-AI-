"""Deep Neural Network Architecture: DeepSVDDAnomalyDetector."""
...
@dataclass
class DeepSVDDAnomalyDetectorInferenceResult:
...
class DeepSVDDAnomalyDetector:
    ...
    def forward(self, x: np.ndarray) -> DeepSVDDAnomalyDetectorInferenceResult:
        if len(x.shape) == 1:
            x = x.reshape(1, -1)
        ...
        return DeepSVDDAnomalyDetectorInferenceResult(
