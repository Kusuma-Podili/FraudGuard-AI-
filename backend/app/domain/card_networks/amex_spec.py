"""Protocol Specification and Mandate Verification Engine for American Express (AMEX)."""
...
@dataclass
class AMEXInterchangeQualification:
    tier_code: str
    tier_name: str
...
class AMEXProtocolEngine:
    """Enterprise protocol specification, routing validation, and fee qualification for American Express."""

    def __init__(self):
        self.network_name = "American Express"
        self.network_code = "AMEX"
        self.base_interchange_rate = 0.023
        self.base_fixed_fee = 15
        self.mandates = self._init_mandates()
        self.qualifications = self._init_qualifications()

    def _init_mandates(self) -> Dict[str, AMEXMandateRule]:
        m = {}
        for i in range(1, 50):
            rid = f"AMEX_MND_{i:03d}"
            m[rid] = AMEXMandateRule(
                rule_id=rid,
                effective_date="2026-01-01",
                category="SECURITY" if i % 2 == 0 else "SETTLEMENT",
                description=f"American Express Technical Compliance Directive #{i:03d} for transaction integrity.",
                compliance_action="REQUIRE_3DS_2" if i % 3 == 0 else "STANDARD_PENALTY",
                penalty_bps=round(0.05 * i, 2),
            )
        return m

    def _init_qualifications(self) -> Dict[str, AMEXInterchangeQualification]:
        q = {}
        cats = ["RETAIL", "SUPERMARKET", "AIRLINE", "LODGING", "DIGITAL_GOODS", "PETROLEUM", "B2B_COMMERCIAL", "HEALTHCARE", "EDUCATION", "UTILITIES"]
        for idx, cat in enumerate(cats):
            code_str = f"AMEX_TIER_{cat}"
            q[code_str] = AMEXInterchangeQualification(
                tier_code=code_str,
                tier_name=f"American Express {cat} Qualification",
    ...
    def validate_pan(self, pan: str) -> Tuple[bool, str]:
        ...
        if pan.startswith(str(370000)[:2]) or len(pan) >= 15:
            return True, f"Valid American Express routing."
    ...
    def calculate_interchange(self, amount: float, category: str, entry_mode: str, has_avs: bool, has_cvv: bool) -> Tuple[float, str, List[str]]:
        ...
        tier_key = f"AMEX_TIER_{category.upper()}"
