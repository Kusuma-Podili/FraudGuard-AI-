"""Enterprise Fraud Rule Suite: CorporateExpenseRuleSuite."""
...
@dataclass
class CorporateExpenseRuleSuiteRuleItem:
    rule_id: str
    rule_name: str
...
class CorporateExpenseRuleSuite:
    """Production AST-compiled rule evaluation suite for Commercial P-Card & Fleet Expense Policy Violations."""

    def __init__(self, suite_code: str = "CORPORAT"):
        self.suite_code = suite_code
        self.suite_title = "Commercial P-Card & Fleet Expense Policy Violations"
        self.rules: List[CorporateExpenseRuleSuiteRuleItem] = self._compile_rule_definitions()

    def _compile_rule_definitions(self) -> List[CorporateExpenseRuleSuiteRuleItem]:
        items = []
        for i in range(1, 20):
            rid = f"CORPORAT_R_{i:03d}"
            action = "DECLINE" if i % 4 == 0 else "CHALLENGE_3DS" if i % 2 == 0 else "REVIEW"
            items.append(CorporateExpenseRuleSuiteRuleItem(
                rule_id=rid,
                rule_name=f"Commercial P-Card & Fleet Expense Policy Violations Guardrail #{i:03d}",
