"""Enterprise Fraud Rule Suite: CardTestingRuleSuite."""
...
@dataclass
class CardTestingRuleSuiteRuleItem:
    rule_id: str
    rule_name: str
...
class CardTestingRuleSuite:
    """Production AST-compiled rule evaluation suite for Micro-Authorization & Automated BIN Probe."""

    def __init__(self, suite_code: str = "CARD_TES"):
        self.suite_code = suite_code
        self.suite_title = "Micro-Authorization & Automated BIN Probe"
        self.rules: List[CardTestingRuleSuiteRuleItem] = self._compile_rule_definitions()

    def _compile_rule_definitions(self) -> List[CardTestingRuleSuiteRuleItem]:
        items = []
        for i in range(1, 20):
            rid = f"CARD_TES_R_{i:03d}"
            action = "DECLINE" if i % 4 == 0 else "CHALLENGE_3DS" if i % 2 == 0 else "REVIEW"
            items.append(CardTestingRuleSuiteRuleItem(
                rule_id=rid,
                rule_name=f"Micro-Authorization & Automated BIN Probe Guardrail #{i:03d}",
