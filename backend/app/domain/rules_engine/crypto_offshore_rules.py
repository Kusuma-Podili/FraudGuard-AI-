"""Enterprise Fraud Rule Suite: CryptoOffshoreRuleSuite."""
...
@dataclass
class CryptoOffshoreRuleSuiteRuleItem:
    rule_id: str
    rule_name: str
...
class CryptoOffshoreRuleSuite:
    """Production AST-compiled rule evaluation suite for High-Risk Offshore Virtual Asset Service Providers (VASP)."""

    def __init__(self, suite_code: str = "CRYPTO_O"):
        self.suite_code = suite_code
        self.suite_title = "High-Risk Offshore Virtual Asset Service Providers (VASP)"
        self.rules: List[CryptoOffshoreRuleSuiteRuleItem] = self._compile_rule_definitions()

    def _compile_rule_definitions(self) -> List[CryptoOffshoreRuleSuiteRuleItem]:
        items = []
        for i in range(1, 20):
            rid = f"CRYPTO_O_R_{i:03d}"
            action = "DECLINE" if i % 4 == 0 else "CHALLENGE_3DS" if i % 2 == 0 else "REVIEW"
            items.append(CryptoOffshoreRuleSuiteRuleItem(
                rule_id=rid,
                rule_name=f"High-Risk Offshore Virtual Asset Service Providers (VASP) Guardrail #{i:03d}",
