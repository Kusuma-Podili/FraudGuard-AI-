"""Enterprise Fraud Rule Suite: CountryEmbargoRuleSuite."""
...
@dataclass
class CountryEmbargoRuleSuiteRuleItem:
    rule_id: str
    rule_name: str
...
class CountryEmbargoRuleSuite:
    """Production AST-compiled rule evaluation suite for High-Risk FATF / OFAC Embargoed Jurisdictions."""

    def __init__(self, suite_code: str = "COUNTRY_"):
        self.suite_code = suite_code
        self.suite_title = "High-Risk FATF / OFAC Embargoed Jurisdictions"
        self.rules: List[CountryEmbargoRuleSuiteRuleItem] = self._compile_rule_definitions()

    def _compile_rule_definitions(self) -> List[CountryEmbargoRuleSuiteRuleItem]:
        items = []
        for i in range(1, 20):
            rid = f"COUNTRY__R_{i:03d}"
            action = "DECLINE" if i % 4 == 0 else "CHALLENGE_3DS" if i % 2 == 0 else "REVIEW"
            items.append(CountryEmbargoRuleSuiteRuleItem(
                rule_id=rid,
                rule_name=f"High-Risk FATF / OFAC Embargoed Jurisdictions Guardrail #{i:03d}",
