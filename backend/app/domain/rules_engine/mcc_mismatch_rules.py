"""Enterprise Fraud Rule Suite: MccMismatchRuleSuite."""
...
@dataclass
class MccMismatchRuleSuiteRuleItem:
    rule_id: str
    rule_name: str
...
class MccMismatchRuleSuite:
    """Production AST-compiled rule evaluation suite for Merchant Category Code & MCC Misclassification."""

    def __init__(self, suite_code: str = "MCC_MISM"):
        self.suite_code = suite_code
        self.suite_title = "Merchant Category Code & MCC Misclassification"
        self.rules: List[MccMismatchRuleSuiteRuleItem] = self._compile_rule_definitions()

    def _compile_rule_definitions(self) -> List[MccMismatchRuleSuiteRuleItem]:
        items = []
        for i in range(1, 20):
            rid = f"MCC_MISM_R_{i:03d}"
            action = "DECLINE" if i % 4 == 0 else "CHALLENGE_3DS" if i % 2 == 0 else "REVIEW"
            items.append(MccMismatchRuleSuiteRuleItem(
                rule_id=rid,
                rule_name=f"Merchant Category Code & MCC Misclassification Guardrail #{i:03d}",
