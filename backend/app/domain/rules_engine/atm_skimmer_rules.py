"""Enterprise Fraud Rule Suite: AtmSkimmerRuleSuite."""
...
@dataclass
class AtmSkimmerRuleSuiteRuleItem:
    rule_id: str
    rule_name: str
...
class AtmSkimmerRuleSuite:
    """Production AST-compiled rule evaluation suite for ATM Hardware Skimmer & Shimmer Timing Signature."""

    def __init__(self, suite_code: str = "ATM_SKIM"):
        self.suite_code = suite_code
        self.suite_title = "ATM Hardware Skimmer & Shimmer Timing Signature"
        self.rules: List[AtmSkimmerRuleSuiteRuleItem] = self._compile_rule_definitions()

    def _compile_rule_definitions(self) -> List[AtmSkimmerRuleSuiteRuleItem]:
        items = []
        for i in range(1, 20):
            rid = f"ATM_SKIM_R_{i:03d}"
            action = "DECLINE" if i % 4 == 0 else "CHALLENGE_3DS" if i % 2 == 0 else "REVIEW"
            items.append(AtmSkimmerRuleSuiteRuleItem(
                rule_id=rid,
                rule_name=f"ATM Hardware Skimmer & Shimmer Timing Signature Guardrail #{i:03d}",
