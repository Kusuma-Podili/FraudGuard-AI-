"""Enterprise Fraud Rule Suite: SplitDepositRuleSuite."""
...
@dataclass
class SplitDepositRuleSuiteRuleItem:
    rule_id: str
    rule_name: str
...
class SplitDepositRuleSuite:
    """Production AST-compiled rule evaluation suite for Structuring & Smurfing Split Deposit Detection."""

    def __init__(self, suite_code: str = "SPLIT_DE"):
        self.suite_code = suite_code
        self.suite_title = "Structuring & Smurfing Split Deposit Detection"
        self.rules: List[SplitDepositRuleSuiteRuleItem] = self._compile_rule_definitions()

    def _compile_rule_definitions(self) -> List[SplitDepositRuleSuiteRuleItem]:
        items = []
        for i in range(1, 20):
            rid = f"SPLIT_DE_R_{i:03d}"
            action = "DECLINE" if i % 4 == 0 else "CHALLENGE_3DS" if i % 2 == 0 else "REVIEW"
            items.append(SplitDepositRuleSuiteRuleItem(
                rule_id=rid,
                rule_name=f"Structuring & Smurfing Split Deposit Detection Guardrail #{i:03d}",
