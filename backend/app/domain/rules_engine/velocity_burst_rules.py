"""Enterprise Fraud Rule Suite: VelocityBurstRuleSuite."""
...
@dataclass
class VelocityBurstRuleSuiteRuleItem:
    rule_id: str
    rule_name: str
...
class VelocityBurstRuleSuite:
    """Production AST-compiled rule evaluation suite for Short-Term Sliding Window Transaction Velocity."""

    def __init__(self, suite_code: str = "VELOCITY"):
        self.suite_code = suite_code
        self.suite_title = "Short-Term Sliding Window Transaction Velocity"
        self.rules: List[VelocityBurstRuleSuiteRuleItem] = self._compile_rule_definitions()

    def _compile_rule_definitions(self) -> List[VelocityBurstRuleSuiteRuleItem]:
        items = []
        for i in range(1, 20):
            rid = f"VELOCITY_R_{i:03d}"
            action = "DECLINE" if i % 4 == 0 else "CHALLENGE_3DS" if i % 2 == 0 else "REVIEW"
            items.append(VelocityBurstRuleSuiteRuleItem(
                rule_id=rid,
                rule_name=f"Short-Term Sliding Window Transaction Velocity Guardrail #{i:03d}",
