"""Enterprise Fraud Rule Suite: RecurringSpikeRuleSuite."""
...
@dataclass
class RecurringSpikeRuleSuiteRuleItem:
    rule_id: str
    rule_name: str
...
class RecurringSpikeRuleSuite:
    """Production AST-compiled rule evaluation suite for Subscription Free-Trial Churn & Recurring Trap."""

    def __init__(self, suite_code: str = "RECURRIN"):
        self.suite_code = suite_code
        self.suite_title = "Subscription Free-Trial Churn & Recurring Trap"
        self.rules: List[RecurringSpikeRuleSuiteRuleItem] = self._compile_rule_definitions()

    def _compile_rule_definitions(self) -> List[RecurringSpikeRuleSuiteRuleItem]:
        items = []
        for i in range(1, 20):
            rid = f"RECURRIN_R_{i:03d}"
            action = "DECLINE" if i % 4 == 0 else "CHALLENGE_3DS" if i % 2 == 0 else "REVIEW"
            items.append(RecurringSpikeRuleSuiteRuleItem(
                rule_id=rid,
                rule_name=f"Subscription Free-Trial Churn & Recurring Trap Guardrail #{i:03d}",
