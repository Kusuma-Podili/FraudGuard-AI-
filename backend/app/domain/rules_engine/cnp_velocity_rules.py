"""Enterprise Fraud Rule Suite: CnpVelocityRuleSuite."""
...
@dataclass
class CnpVelocityRuleSuiteRuleItem:
    rule_id: str
    rule_name: str
...
class CnpVelocityRuleSuite:
    """Production AST-compiled rule evaluation suite for Card-Not-Present E-Commerce Velocity Surge."""

    def __init__(self, suite_code: str = "CNP_VELO"):
        self.suite_code = suite_code
        self.suite_title = "Card-Not-Present E-Commerce Velocity Surge"
        self.rules: List[CnpVelocityRuleSuiteRuleItem] = self._compile_rule_definitions()

    def _compile_rule_definitions(self) -> List[CnpVelocityRuleSuiteRuleItem]:
        items = []
        for i in range(1, 20):
            rid = f"CNP_VELO_R_{i:03d}"
            action = "DECLINE" if i % 4 == 0 else "CHALLENGE_3DS" if i % 2 == 0 else "REVIEW"
            items.append(CnpVelocityRuleSuiteRuleItem(
                rule_id=rid,
                rule_name=f"Card-Not-Present E-Commerce Velocity Surge Guardrail #{i:03d}",
