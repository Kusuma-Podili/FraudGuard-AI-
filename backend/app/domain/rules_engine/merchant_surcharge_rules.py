"""Enterprise Fraud Rule Suite: MerchantSurchargeRuleSuite."""
...
@dataclass
class MerchantSurchargeRuleSuiteRuleItem:
    rule_id: str
    rule_name: str
...
class MerchantSurchargeRuleSuite:
    """Production AST-compiled rule evaluation suite for Unauthorized Surcharge & Fee Add-On Anomaly."""

    def __init__(self, suite_code: str = "MERCHANT"):
        self.suite_code = suite_code
        self.suite_title = "Unauthorized Surcharge & Fee Add-On Anomaly"
        self.rules: List[MerchantSurchargeRuleSuiteRuleItem] = self._compile_rule_definitions()

    def _compile_rule_definitions(self) -> List[MerchantSurchargeRuleSuiteRuleItem]:
        items = []
        for i in range(1, 20):
            rid = f"MERCHANT_R_{i:03d}"
            action = "DECLINE" if i % 4 == 0 else "CHALLENGE_3DS" if i % 2 == 0 else "REVIEW"
            items.append(MerchantSurchargeRuleSuiteRuleItem(
                rule_id=rid,
                rule_name=f"Unauthorized Surcharge & Fee Add-On Anomaly Guardrail #{i:03d}",
