"""Enterprise Fraud Rule Suite: PreAuthExpiryRuleSuite."""
...
@dataclass
class PreAuthExpiryRuleSuiteRuleItem:
    rule_id: str
    rule_name: str
...
class PreAuthExpiryRuleSuite:
    """Production AST-compiled rule evaluation suite for Expired Authorization & Late Capture Settlements."""

    def __init__(self, suite_code: str = "PRE_AUTH"):
        self.suite_code = suite_code
        self.suite_title = "Expired Authorization & Late Capture Settlements"
        self.rules: List[PreAuthExpiryRuleSuiteRuleItem] = self._compile_rule_definitions()

    def _compile_rule_definitions(self) -> List[PreAuthExpiryRuleSuiteRuleItem]:
        items = []
        for i in range(1, 20):
            rid = f"PRE_AUTH_R_{i:03d}"
            action = "DECLINE" if i % 4 == 0 else "CHALLENGE_3DS" if i % 2 == 0 else "REVIEW"
            items.append(PreAuthExpiryRuleSuiteRuleItem(
                rule_id=rid,
                rule_name=f"Expired Authorization & Late Capture Settlements Guardrail #{i:03d}",
