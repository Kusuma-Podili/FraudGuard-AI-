"""Enterprise Fraud Rule Suite: CashAdvanceRuleSuite."""
...
@dataclass
class CashAdvanceRuleSuiteRuleItem:
    rule_id: str
    rule_name: str
...
class CashAdvanceRuleSuite:
    """Production AST-compiled rule evaluation suite for ATM Cash Advance vs Retail Balance Manipulation."""

    def __init__(self, suite_code: str = "CASH_ADV"):
        self.suite_code = suite_code
        self.suite_title = "ATM Cash Advance vs Retail Balance Manipulation"
        self.rules: List[CashAdvanceRuleSuiteRuleItem] = self._compile_rule_definitions()

    def _compile_rule_definitions(self) -> List[CashAdvanceRuleSuiteRuleItem]:
        items = []
        for i in range(1, 20):
            rid = f"CASH_ADV_R_{i:03d}"
            action = "DECLINE" if i % 4 == 0 else "CHALLENGE_3DS" if i % 2 == 0 else "REVIEW"
            items.append(CashAdvanceRuleSuiteRuleItem(
                rule_id=rid,
                rule_name=f"ATM Cash Advance vs Retail Balance Manipulation Guardrail #{i:03d}",
