"""Enterprise Fraud Rule Suite: BinAttackRuleSuite."""
...
@dataclass
class BinAttackRuleSuiteRuleItem:
    rule_id: str
    rule_name: str
...
class BinAttackRuleSuite:
    """Production AST-compiled rule evaluation suite for Sequential Pan Enumeration & Brute-Force Testing."""

    def __init__(self, suite_code: str = "BIN_ATTA"):
        self.suite_code = suite_code
        self.suite_title = "Sequential Pan Enumeration & Brute-Force Testing"
        self.rules: List[BinAttackRuleSuiteRuleItem] = self._compile_rule_definitions()

    def _compile_rule_definitions(self) -> List[BinAttackRuleSuiteRuleItem]:
        items = []
        for i in range(1, 20):
            rid = f"BIN_ATTA_R_{i:03d}"
            action = "DECLINE" if i % 4 == 0 else "CHALLENGE_3DS" if i % 2 == 0 else "REVIEW"
            items.append(BinAttackRuleSuiteRuleItem(
                rule_id=rid,
                rule_name=f"Sequential Pan Enumeration & Brute-Force Testing Guardrail #{i:03d}",
