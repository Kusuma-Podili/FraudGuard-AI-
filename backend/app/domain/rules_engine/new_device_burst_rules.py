"""Enterprise Fraud Rule Suite: NewDeviceBurstRuleSuite."""
...
@dataclass
class NewDeviceBurstRuleSuiteRuleItem:
    rule_id: str
    rule_name: str
...
class NewDeviceBurstRuleSuite:
    """Production AST-compiled rule evaluation suite for Unrecognized Hardware Fingerprint Velocity."""

    def __init__(self, suite_code: str = "NEW_DEVI"):
        self.suite_code = suite_code
        self.suite_title = "Unrecognized Hardware Fingerprint Velocity"
        self.rules: List[NewDeviceBurstRuleSuiteRuleItem] = self._compile_rule_definitions()

    def _compile_rule_definitions(self) -> List[NewDeviceBurstRuleSuiteRuleItem]:
        items = []
        for i in range(1, 20):
            rid = f"NEW_DEVI_R_{i:03d}"
            action = "DECLINE" if i % 4 == 0 else "CHALLENGE_3DS" if i % 2 == 0 else "REVIEW"
            items.append(NewDeviceBurstRuleSuiteRuleItem(
                rule_id=rid,
                rule_name=f"Unrecognized Hardware Fingerprint Velocity Guardrail #{i:03d}",
