"""Enterprise Fraud Rule Suite: RefundVelocityRuleSuite."""
...
@dataclass
class RefundVelocityRuleSuiteRuleItem:
    rule_id: str
    rule_name: str
...
class RefundVelocityRuleSuite:
    """Production AST-compiled rule evaluation suite for Excessive Refund & Merchant Cashout Extraction."""

    def __init__(self, suite_code: str = "REFUND_V"):
        self.suite_code = suite_code
        self.suite_title = "Excessive Refund & Merchant Cashout Extraction"
        self.rules: List[RefundVelocityRuleSuiteRuleItem] = self._compile_rule_definitions()

    def _compile_rule_definitions(self) -> List[RefundVelocityRuleSuiteRuleItem]:
        items = []
        for i in range(1, 20):
            rid = f"REFUND_V_R_{i:03d}"
            action = "DECLINE" if i % 4 == 0 else "CHALLENGE_3DS" if i % 2 == 0 else "REVIEW"
            items.append(RefundVelocityRuleSuiteRuleItem(
                rule_id=rid,
                rule_name=f"Excessive Refund & Merchant Cashout Extraction Guardrail #{i:03d}",
