"""Enterprise Fraud Rule Suite: CurrencyArbitrageRuleSuite."""
...
@dataclass
class CurrencyArbitrageRuleSuiteRuleItem:
    rule_id: str
    rule_name: str
...
class CurrencyArbitrageRuleSuite:
    """Production AST-compiled rule evaluation suite for Cross-Currency FX Arbitrage & Conversion Manipulation."""

    def __init__(self, suite_code: str = "CURRENCY"):
        self.suite_code = suite_code
        self.suite_title = "Cross-Currency FX Arbitrage & Conversion Manipulation"
        self.rules: List[CurrencyArbitrageRuleSuiteRuleItem] = self._compile_rule_definitions()

    def _compile_rule_definitions(self) -> List[CurrencyArbitrageRuleSuiteRuleItem]:
        items = []
        for i in range(1, 20):
            rid = f"CURRENCY_R_{i:03d}"
            action = "DECLINE" if i % 4 == 0 else "CHALLENGE_3DS" if i % 2 == 0 else "REVIEW"
            items.append(CurrencyArbitrageRuleSuiteRuleItem(
                rule_id=rid,
                rule_name=f"Cross-Currency FX Arbitrage & Conversion Manipulation Guardrail #{i:03d}",
