"""Enterprise Fraud Rule Suite: OffHoursSpikeRuleSuite."""
...
@dataclass
class OffHoursSpikeRuleSuiteRuleItem:
    rule_id: str
    rule_name: str
...
class OffHoursSpikeRuleSuite:
    """Production AST-compiled rule evaluation suite for Nocturnal & Unusual Temporal Spending Hours."""

    def __init__(self, suite_code: str = "OFF_HOUR"):
        self.suite_code = suite_code
        self.suite_title = "Nocturnal & Unusual Temporal Spending Hours"
        self.rules: List[OffHoursSpikeRuleSuiteRuleItem] = self._compile_rule_definitions()

    def _compile_rule_definitions(self) -> List[OffHoursSpikeRuleSuiteRuleItem]:
        items = []
        for i in range(1, 20):
            rid = f"OFF_HOUR_R_{i:03d}"
            action = "DECLINE" if i % 4 == 0 else "CHALLENGE_3DS" if i % 2 == 0 else "REVIEW"
            items.append(OffHoursSpikeRuleSuiteRuleItem(
                rule_id=rid,
                rule_name=f"Nocturnal & Unusual Temporal Spending Hours Guardrail #{i:03d}",
