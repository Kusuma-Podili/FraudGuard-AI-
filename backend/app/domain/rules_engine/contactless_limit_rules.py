"""Enterprise Fraud Rule Suite: ContactlessLimitRuleSuite."""
...
@dataclass
class ContactlessLimitRuleSuiteRuleItem:
    rule_id: str
    rule_name: str
...
class ContactlessLimitRuleSuite:
    """Production AST-compiled rule evaluation suite for NFC Tap-to-Pay Cumulative Offline Floor Limits."""

    def __init__(self, suite_code: str = "CONTACTL"):
        self.suite_code = suite_code
        self.suite_title = "NFC Tap-to-Pay Cumulative Offline Floor Limits"
        self.rules: List[ContactlessLimitRuleSuiteRuleItem] = self._compile_rule_definitions()

    def _compile_rule_definitions(self) -> List[ContactlessLimitRuleSuiteRuleItem]:
        items = []
        for i in range(1, 20):
            rid = f"CONTACTL_R_{i:03d}"
            action = "DECLINE" if i % 4 == 0 else "CHALLENGE_3DS" if i % 2 == 0 else "REVIEW"
            items.append(ContactlessLimitRuleSuiteRuleItem(
                rule_id=rid,
                rule_name=f"NFC Tap-to-Pay Cumulative Offline Floor Limits Guardrail #{i:03d}",
