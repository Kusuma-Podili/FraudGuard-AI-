"""Enterprise Fraud Rule Suite: FriendlyFraudRuleSuite."""
...
@dataclass
class FriendlyFraudRuleSuiteRuleItem:
    rule_id: str
    rule_name: str
...
class FriendlyFraudRuleSuite:
    """Production AST-compiled rule evaluation suite for First-Party Friendly Fraud & Abuse of Dispute Rights."""

    def __init__(self, suite_code: str = "FRIENDLY"):
        self.suite_code = suite_code
        self.suite_title = "First-Party Friendly Fraud & Abuse of Dispute Rights"
        self.rules: List[FriendlyFraudRuleSuiteRuleItem] = self._compile_rule_definitions()

    def _compile_rule_definitions(self) -> List[FriendlyFraudRuleSuiteRuleItem]:
        items = []
        for i in range(1, 20):
            rid = f"FRIENDLY_R_{i:03d}"
            action = "DECLINE" if i % 4 == 0 else "CHALLENGE_3DS" if i % 2 == 0 else "REVIEW"
            items.append(FriendlyFraudRuleSuiteRuleItem(
                rule_id=rid,
                rule_name=f"First-Party Friendly Fraud & Abuse of Dispute Rights Guardrail #{i:03d}",
