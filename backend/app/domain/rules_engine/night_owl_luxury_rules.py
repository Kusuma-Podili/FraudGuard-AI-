"""Enterprise Fraud Rule Suite: NightOwlLuxuryRuleSuite."""
...
@dataclass
class NightOwlLuxuryRuleSuiteRuleItem:
    rule_id: str
    rule_name: str
...
class NightOwlLuxuryRuleSuite:
    """Production AST-compiled rule evaluation suite for High-Ticket Jewelry & Watch Purchases at 3 AM."""

    def __init__(self, suite_code: str = "NIGHT_OW"):
        self.suite_code = suite_code
        self.suite_title = "High-Ticket Jewelry & Watch Purchases at 3 AM"
        self.rules: List[NightOwlLuxuryRuleSuiteRuleItem] = self._compile_rule_definitions()

    def _compile_rule_definitions(self) -> List[NightOwlLuxuryRuleSuiteRuleItem]:
        items = []
        for i in range(1, 20):
            rid = f"NIGHT_OW_R_{i:03d}"
            action = "DECLINE" if i % 4 == 0 else "CHALLENGE_3DS" if i % 2 == 0 else "REVIEW"
            items.append(NightOwlLuxuryRuleSuiteRuleItem(
                rule_id=rid,
                rule_name=f"High-Ticket Jewelry & Watch Purchases at 3 AM Guardrail #{i:03d}",
