"""Enterprise Fraud Rule Suite: IpProxyReputationRuleSuite."""
...
@dataclass
class IpProxyReputationRuleSuiteRuleItem:
    rule_id: str
    rule_name: str
...
class IpProxyReputationRuleSuite:
    """Production AST-compiled rule evaluation suite for VPN / Commercial Proxy & Anonymizer Detection."""

    def __init__(self, suite_code: str = "IP_PROXY"):
        self.suite_code = suite_code
        self.suite_title = "VPN / Commercial Proxy & Anonymizer Detection"
        self.rules: List[IpProxyReputationRuleSuiteRuleItem] = self._compile_rule_definitions()

    def _compile_rule_definitions(self) -> List[IpProxyReputationRuleSuiteRuleItem]:
        items = []
        for i in range(1, 20):
            rid = f"IP_PROXY_R_{i:03d}"
            action = "DECLINE" if i % 4 == 0 else "CHALLENGE_3DS" if i % 2 == 0 else "REVIEW"
            items.append(IpProxyReputationRuleSuiteRuleItem(
                rule_id=rid,
                rule_name=f"VPN / Commercial Proxy & Anonymizer Detection Guardrail #{i:03d}",
