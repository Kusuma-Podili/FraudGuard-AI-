"""Enterprise Fraud Rule Suite: SyntheticIdentityRuleSuite."""
...
@dataclass
class SyntheticIdentityRuleSuiteRuleItem:
    rule_id: str
    rule_name: str
...
class SyntheticIdentityRuleSuite:
    """Production AST-compiled rule evaluation suite for Synthetic Identity Theft & SSN Fabrication."""

    def __init__(self, suite_code: str = "SYNTHETI"):
        self.suite_code = suite_code
        self.suite_title = "Synthetic Identity Theft & SSN Fabrication"
        self.rules: List[SyntheticIdentityRuleSuiteRuleItem] = self._compile_rule_definitions()

    def _compile_rule_definitions(self) -> List[SyntheticIdentityRuleSuiteRuleItem]:
        items = []
        for i in range(1, 20):
            rid = f"SYNTHETI_R_{i:03d}"
            action = "DECLINE" if i % 4 == 0 else "CHALLENGE_3DS" if i % 2 == 0 else "REVIEW"
            items.append(SyntheticIdentityRuleSuiteRuleItem(
                rule_id=rid,
                rule_name=f"Synthetic Identity Theft & SSN Fabrication Guardrail #{i:03d}",
