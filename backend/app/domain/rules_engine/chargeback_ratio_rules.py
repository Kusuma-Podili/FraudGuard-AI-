"""Enterprise Fraud Rule Suite: ChargebackRatioRuleSuite."""
...
@dataclass
class ChargebackRatioRuleSuiteRuleItem:
    rule_id: str
    rule_name: str
...
class ChargebackRatioRuleSuite:
    """Production AST-compiled rule evaluation suite for Visa VFMP / Mastercard ECP Threshold Breaches."""

    def __init__(self, suite_code: str = "CHARGEBA"):
        self.suite_code = suite_code
        self.suite_title = "Visa VFMP / Mastercard ECP Threshold Breaches"
        self.rules: List[ChargebackRatioRuleSuiteRuleItem] = self._compile_rule_definitions()

    def _compile_rule_definitions(self) -> List[ChargebackRatioRuleSuiteRuleItem]:
        items = []
        for i in range(1, 20):
            rid = f"CHARGEBA_R_{i:03d}"
            action = "DECLINE" if i % 4 == 0 else "CHALLENGE_3DS" if i % 2 == 0 else "REVIEW"
            items.append(ChargebackRatioRuleSuiteRuleItem(
                rule_id=rid,
                rule_name=f"Visa VFMP / Mastercard ECP Threshold Breaches Guardrail #{i:03d}",
