...
export default function SettingsPage() {
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-bold text-[#111827] tracking-tight">System Settings & Risk Policies</h1>
          </div>
      ...
      {successMessage && (
        <div className="p-3.5 bg-gray-100 border border-gray-300 rounded-xl text-xs text-gray-900 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <CheckCircle2 className="w-4 h-4 text-gray-900 shrink-0" />
            <span>{successMessage}</span>
          </div>
          <button onClick={() => setSuccessMessage(null)} className="text-[#4B5563] hover:text-[#111827]">✕</button>
        </div>
      )}
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <span className="text-[11px] px-2.5 py-1 rounded bg-gray-100 text-gray-800 font-semibold border border-gray-300 flex items-center gap-1.5">
        ...
        <div className="p-5 pt-0 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {[
            ...
            const Icon = srv.icon;
            return (
              <div key={idx} className="p-3.5 bg-[#F9FAFB] border border-[#E5E7EB] rounded-xl space-y-2 text-center">
                <div className="w-8 h-8 rounded-lg bg-gray-100 text-gray-800 mx-auto flex items-center justify-center">
                  <Icon className="w-4 h-4" />
                </div>
                <div>
                  <p className="text-[11px] font-bold text-[#111827]">{srv.name}</p>
                  <span className="text-[10px] font-bold text-gray-600 uppercase">{srv.status}</span>
                </div>
              </div>
            );
          })}
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              <div className="flex justify-between text-xs">
                <span className="font-semibold text-gray-700">Low Risk Upper Bound (ALLOW)</span>
                <span className="font-mono text-[#111827]">{thresholds.low_max.toFixed(2)}</span>
              </div>
              <input
                type="range"
                min="0.10"
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between text-xs">
                <span className="font-semibold text-gray-700">Medium Risk Upper Bound (REVIEW)</span>
                <span className="font-mono text-[#111827]">{thresholds.medium_max.toFixed(2)}</span>
              </div>
              <input
                type="range"
                min="0.40"
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between text-xs">
                <span className="font-semibold text-[#EA580C]">Critical Risk Lower Bound (DECLINE)</span>
                <span className="font-mono text-[#111827]">{thresholds.critical_min.toFixed(2)}</span>
              </div>
              <input
                type="range"
                min="0.70"
            ...
            <div className="pt-2 flex items-center justify-between border-t border-[#E5E7EB]">
              <div>
                <p className="text-xs font-bold text-[#111827]">Automated Decline Enforcer</p>
                <p className="text-[10px] text-[#4B5563]">Auto-reject transactions exceeding critical threshold.</p>
              </div>
              <input
                type="checkbox"
                checked={thresholds.auto_decline_enabled}
                onChange={(e) => setThresholds({ ...thresholds, auto_decline_enabled: e.target.checked })}
                className="w-4 h-4 accent-[#FB923C] rounded cursor-pointer"
              />
            </div>
        ...
        <Card className="space-y-5 p-6 bg-white">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between py-2 border-b border-[#E5E7EB]">
              <div>
                <p className="text-xs font-bold text-[#111827]">In-App Notification Center</p>
                <p className="text-[10px] text-[#4B5563]">Display instant badges and notification toasts.</p>
              </div>
              <input
                type="checkbox"
            ...
            <div className="flex items-center justify-between py-2 border-b border-[#E5E7EB]">
              <div>
                <p className="text-xs font-bold text-[#111827]">Critical Audio Chime</p>
                <p className="text-[10px] text-[#4B5563]">Audible notification for critical severity threats.</p>
              </div>
              <input
                type="checkbox"
            ...
            <div className="space-y-1.5 pt-1">
              <label className="text-xs font-semibold text-[#111827]">Minimum Alert Severity Trigger</label>
              <select
            ...
            <div className="space-y-1.5 pt-1">
              <label className="text-xs font-semibold text-[#111827]">Slack / SIEM Webhook URL</label>
              <input
                type="text"
